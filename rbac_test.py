#!/usr/bin/env python3
"""
SEC-P0-02 RBAC Backend Matrix Test Suite
...
Tests the following matrix:
1. GET /api/v1/players (list) - Support: 200, Ops: 200, Tenant Admin: 200, Super Admin: 200
2. POST /api/v1/players/{player_id}/credit - Tenant Admin: 200, Super Admin: 200, Ops: 403, Support: 403
3. POST /api/v1/players/{player_id}/debit - Tenant Admin: 200, Super Admin: 200, Ops: 403, Support: 403
4. POST /api/v1/players/{player_id}/suspend - Ops: 200, Tenant Admin: 200, Super Admin: 200, Support: 403
5. POST /api/v1/players/{player_id}/unsuspend - Ops: 200, Tenant Admin: 200, Super Admin: 200, Support: 403
6. POST /api/v1/bonuses/grant - Tenant Admin: 200, Super Admin: 200, Ops: 403, Support: 403
7. POST /api/v1/bonuses/{grant_id}/revoke - Tenant Admin: 200, Super Admin: 200, Ops: 403, Support: 403
8. POST /api/v1/bonuses/{grant_id}/expire - Tenant Admin: 200, Super Admin: 200, Ops: 403, Support: 403
9. POST /api/v1/affiliates/payouts - Ops: 200, Tenant Admin: 200, Super Admin: 200, Support: 403

Uses external ingress base URL from REACT_APP_BACKEND_URL.
"""
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        ...
                        "X-Reason": f"RBAC test credit by {role_name}"
                    }
                    # Only Super Admin can use X-Tenant-ID header for impersonation
                    if role_name == "Super Admin":
                        headers["X-Tenant-ID"] = "default_casino"
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        ...
                        "X-Reason": f"RBAC test suspend by {role_name}"
                    }
                    payload = {}
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "X-Tenant-ID": "default_casino",
                        "X-Reason": f"RBAC test unsuspend by {role_name}"
                    }
                    payload = {}
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                headers = {
                    "Authorization": f"Bearer {self.super_admin_token}",
                    "X-Tenant-ID": "default_casino",
                    "X-Reason": "RBAC test campaign creation"
                }
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "X-Tenant-ID": "default_casino",
                        "X-Reason": f"RBAC test bonus grant by {role_name}"
                    }
                    payload = {
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "X-Tenant-ID": "default_casino",
                        "X-Reason": f"RBAC test bonus revoke by {role_name}"
                    }
                    payload = {}
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "X-Tenant-ID": "default_casino",
                        "X-Reason": f"RBAC test bonus expire by {role_name}"
                    }
                    payload = {}
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for role_name, token, expected_status in roles:
                    ...
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "X-Tenant-ID": "default_casino",
                        "X-Reason": f"RBAC test affiliate payout by {role_name}"
                    }
                    payload = {
