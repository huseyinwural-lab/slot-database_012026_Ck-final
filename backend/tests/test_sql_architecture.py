...
async def test_sql_logic():
    ...
    async with async_session() as session:
        ...
        # Verify JSON Feature
        if fetched_admin.tenant and fetched_admin.tenant.features.get("robot") is True:
             # Note: relationship loading might require explicit eager load in async, 
             # but let's check simple field access first.
             pass
