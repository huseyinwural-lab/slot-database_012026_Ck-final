...
async def run_game_sim(config: GameSimRequest):
    ...
    for i in range(config.count):
        ...
        await db.transactions.insert_one(Transaction(
            ...
            player_id=player['id'],
            type=TransactionType.ADJUSTMENT, # Using adjustment for generic game bet
            amount=-bet,
        ...
        logs.append(SimulationLog(
            type="game_sim", 
            message=f"Spin for {player['username']}: {'WIN' if is_win else 'LOSS'}", 
            details={"bet": bet, "win": win_amount}
        ))
        await asyncio.sleep(config.delay_ms / 1000)
...
async def test_finance_callback(req: FinanceSimRequest):
    ...
    # Don't save to DB to avoid pollution, just return result for UI log
    return {
        "transaction": tx,
        "provider_response": {
...
async def time_travel(req: TimeTravelRequest):
    # In a real app, we'd offset the system clock or DB queries. 
