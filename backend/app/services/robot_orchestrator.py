...
class RobotOrchestrator:
  def __init__(self, db: AsyncIOMotorDatabase, http_client: httpx.AsyncClient):
      self.db = db
  ...
  async def _find_test_game(self, tenant_id: str, game_type: str) -> Optional[Dict[str, Any]]:
      # Basit yaklaşım: games koleksiyonunda kategoriye ve tenant_id'ye göre bir oyun bul
      query = {
          "tenant_id": tenant_id,
          "category": game_type.capitalize(),  # Slot, Crash, Dice
      }
      game = await self.db.games.find_one(query)
      return game
